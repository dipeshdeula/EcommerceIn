...
namespace Application.Features.AddressFeat.Commands
{
    public class AddressCommandValidator : AbstractValidator<AddressCommand>
    {
        ...
        public AddressCommandValidator(IUserRepository userRepository, IAddressRepository addressRepository)
        {
            ...
            RuleFor(x => x.addressDTO.Province)
            ...
            .Must(lat =>
            {
                if (!double.TryParse(lat.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out double latitude))
                    return false;
            ...
            RuleFor(x => x.addressDTO.Longitude)
                ...
                .Must(lon =>
                {
                    if (!double.TryParse(lon.ToString(), NumberStyles.Any, CultureInfo.InvariantCulture, out double longitude))
                        return false;
